...
# Evaluation
from train_eval_class import evaluate_one,evaluate_fft
...
#
    ...
    net = UNet_Dual(in_channels=1, out_channels=24, bilinear=True, classification=False)
    config['lr'] = 0.005

    print('the number of trainable parameters: %d' % get_number_of_learnable_parameters(net))
    print('Initial learning rate :',config['lr'])
    config['checkout'] = '/public/huangjunzhang/KeyPointsDetection-master/Checkpoints_2308/Dual/{}'.format(
        args.seed)
    ...
    for epoch in range(config['start_epoch'],config['epoch_num']+config['start_epoch']):
        ...
        for i, (inputs, info) in enumerate(trainDataLoader):
            ...
            #lam = 0.05 * epoch if epoch >20 else 0 #our parameter 230804really
            ...
            # final version
            running_loss = kp_loss +regression_loss_y + regression_loss_x+ class_loss
            ...
            if (i+1) % config['eval_freq'] == 0:
                ...
                print('[ Epoch {:005d} -> {:005d} / {} ] loss : {:15} CLASSLOSS:{} max : {:10} min : {}'.format(
                    ...
                    sample_num, running_loss.item(),running_loss.item(),v_max.item(),v_min.item()))
        ...
        with torch.no_grad():
            ...
            if mean_auc > best_auc :
                torch.save(net.module.state_dict() if len(gpus) > 1 else net.state_dict(),
                           config['checkout'] + 'kd_net{model}_{seed}__{lr}bestFFT_model.ckpt'.format(model=args.keypoint_model,seed=args.seed, lr=config['lr']))
                best_auc = mean_auc
                best_epoch = epoch
        ...
        if args.attention:
            torch.save(net.module.state_dict() if len(gpus) > 1 else net.state_dict(),
                   config['checkout']+'epoch_{model}{epoch}_{lr}_fft{seed}.ckpt'.format(model=args.keypoint_model,epoch=epoch, lr=config['lr'], seed=args.seed))
        else:
            torch.save(net.module.state_dict() if len(gpus) > 1 else net.state_dict(),
                   config['checkout']+'epoch_{model}{epoch}__{lr}_fft{seed}.ckpt'.format(model=args.keypoint_model,epoch=epoch, lr=config['lr'], seed=args.seed))

    plt.figure()
    plt.plot(train_loss, 'b-', label='Recon_loss')
    plt.ylabel('Train_loss')
    plt.xlabel('iter_num')
    plt.savefig(config['checkout']+'loss{SEED}_{lr}.jpg'.format(SEED=args.seed, lr=config['lr']))
