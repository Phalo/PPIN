...
##Model Selection
from MMWUnet_lumbar import MMWUNet
...
if __name__ == '__main__':
    ...
    parser.add_argument("--keypoint_model", type=str, default='baseline_l',
                        help="the model name"
                             "2020shen"
                             "YNet"
                             "2020Wang "
    ...
    parser.add_argument('--keypoint_learning_rate', type=float, default=0.002,
...
#
    ...
    if args.keypoint_model == '2020shen':
        ...
        net = MMWUNet(in_channels=1, out_channels=20, classification=True, classification_gcn=True,
                    classification_only=False)
        #config['lr'] = 0.005
        #config['checkout'] = '/public/huangjunzhang/KeyPointsDetection-master/Checkpoints_final/ablation/ALL/'

    else:
        net = UNet_base(in_channels=1, out_channels=20, bilinear=True, classification=True)
        config['lr'] = 0.002
    ...
    config['checkout'] = '/public/huangjunzhang/KeyPointsDetection-master/Checkpoints_2308/baseline_lumbar/{}'.format(
        args.seed)
    ...
    trainDataset = KFDataset(config , mode='train', transforms=data_transforms["train"],fold= None,lumbar=True)
    # 定义 data loader
    ...
    valDataset = KFDataset(config , mode='test', transforms=data_transforms["val"],fold= None,lumbar=True)
    ...
    for epoch in range(config['start_epoch'],config['epoch_num']+config['start_epoch']):
        ...
        for i, (inputs, info) in enumerate(trainDataLoader):
            lam = 0.05 * epoch if epoch> 20 else 1
            ...
            # final version
            labels = torch.max(info["label"].reshape(-1, 5, 4), dim=2).values
        ...
        with torch.no_grad():
            dic, summary, mean_auc, mean_recall=evaluate_one(model=net, dataloader=valDataLoader,lumbar=True)
            ...
            if mean_auc > best_auc :
                torch.save(net.module.state_dict() if len(gpus) > 1 else net.state_dict(),
                           config['checkout'] + 'lumbar_net{model}_{seed}_bestAG_model.ckpt'.format(model=args.keypoint_model,seed=args.seed))
                best_auc = mean_auc
                best_epoch = epoch
        ...
        if args.attention:
            torch.save(net.module.state_dict() if len(gpus) > 1 else net.state_dict(),
                   config['checkout']+'lumbar_epoch_{model}{epoch}_{lr}_AG{seed}.ckpt'.format(model=args.keypoint_model,epoch=epoch, lr=config['lr'], seed=args.seed))
        else:
            torch.save(net.module.state_dict() if len(gpus) > 1 else net.state_dict(),
                   config['checkout']+'lumbar_epoch_{model}{epoch}_{lr}_{seed}.ckpt'.format(model=args.keypoint_model,epoch=epoch, lr=config['lr'], seed=args.seed))
    ...
    plt.xlabel('iter_num')
    plt.savefig(config['checkout']+'lumbar_loss{SEED}.jpg'.format(SEED=args.seed))
