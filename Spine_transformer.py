...
from matcher import build_matcher,generalized_box_iou,box_cxcywh_to_xyxy,box_xyxy_to_cxcywh
...
class Classification_sim(nn.Module):
    def __init__(self,batch_size=1, num_classes=1):
        ...
        print('using groupclassifer for training')
        self.avgpool2d = nn.AdaptiveMaxPool2d((24,1))
        self.avgpool1d = nn.AdaptiveMaxPool1d(1)
        self.fc = nn.ModuleList(nn.Linear(512,1)for i in range(6))
    ...
    def forward(self,x):
        ...
        #         x_sim = x_sim+x_diff
            ...
            x_sim = x[:, :,i]
            x_sim=x_sim.squeeze().view(-1,512)
...
class SpineTransformer(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, aux_loss=False):
        ...
        #self.res_proj  = nn.Linear(256,num_queries)
        self.res_proj = nn.Linear(49, num_queries)
        self.backbone = backbone
        self.aux_loss = aux_loss
        self.classifer = Classification_sim()
    def forward(self, samples: NestedTensor):
        ...
        hs_orig = self.res_proj(self.input_proj(src).flatten(2)).unsqueeze(0).repeat(2,1,1,1).permute(0,1,3,2)
        hs = self.transformer(self.input_proj(src), mask, self.query_embed.weight, pos[-1])[0]
        hs = hs_orig+hs
        outputs_class = self.class_embed(hs).sigmoid()
        #outputs_sim = self.classifer(hs_orig)
        #outputs_class = torch.max(outputs_class)
